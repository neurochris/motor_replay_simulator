...
def generate_awake_data():
    ...
    return binmat, df, df_seq_final

def generate_sim_mat(binmat):
    a = 0.5
    window = 25

    es = editsim.FromBinMat(alpha=a)
    es.gensimmat(binmat, window=window, slide=10, numband=500, minhash=False)
...
def main():
    binmat, df, df_seq_final = generate_awake_data()

    es = generate_sim_mat(binmat)

    my_dict = detect_clusters(es)

    plot_template(es, my_dict)
